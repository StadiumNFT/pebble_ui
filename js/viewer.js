...
export async function initViewer(mods) {
  ...
  const w = document.getElementById('viewer').clientWidth || 600;
  const h = document.getElementById('viewer').clientHeight || 400;
  ...
  const groundMat = new THREE.MeshStandardMaterial({ color: 0xf0f4ff });
...
async function loadGLB(url) {
  ...
  return new Promise((resolve, reject) => {
    loader.load(url, (gltf) => {
      ...
      pebble = gltf.scene;
      pebble.traverse((o) => { if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; } });
...
// Simple idle bounce + talk wiggle
function animate() {
  ...
  if (pebble) {
    ...
    // If talking, add a gentle wiggle to simulate mouth movement
    if (isTalking) {
